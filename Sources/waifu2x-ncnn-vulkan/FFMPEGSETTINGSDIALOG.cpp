...
BEGIN_MESSAGE_MAP(FFMPEGSETTINGSDIALOG, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_CODEC, &FFMPEGSETTINGSDIALOG::OnBnClickedButtonCodec)
END_MESSAGE_MAP()
...
BOOL FFMPEGSETTINGSDIALOG::OnInitDialog()
{
	...
	if (preset == 0) {
		...
		FINALPARAM = L"ffmpeg" + FPS + L" -hide_banner" + L" -strict -2" + L" -i $InFile1" + L" -i $InFile2" + FPS2 + L" -qp 0" + L" -vcodec hevc_nvenc" + L" -acodec aac" + L" -b:a 192k" + L" -y" + L" $OutFile";
		CButton* abutton = (CButton*)GetDlgItem(IDC_CHECK_ADVANCED);
		CButton* codecbutton = (CButton*)GetDlgItem(IDC_BUTTON_CODEC);
		...
		codecbutton->EnableWindow(FALSE);
		...
		if (abutton->GetCheck() == BST_UNCHECKED) {
			CUSTOMPARAM = L"ffmpeg" + FPS + L" -hide_banner" + L" -strict -2" + L" -i $InFile1" + L" -i $InFile2" + FPS2 + L" -qp 0" + VIDEOCODEC + AUDIOCODEC + AUDIOBITRATE + L" -y" + L" $OutFile";
		}
		else {
			CUSTOMPARAM = L"ffmpeg" + FPS + HIDEBANNER + STRCT + L" -i $InFile1" + L" -i $InFile2" + FPS2 + QP + VIDEOCODEC + AUDIOCODEC + AUDIOBITRATE + OVERWRITE + L" $OutFile";
		}
	}
	else {
		m_PRESET.SetCurSel(0);
...
void FFMPEGSETTINGSDIALOG::OnBnClickedButtonCodec()
{
	// TODO: ここにコントロール通知ハンドラー コードを追加します。
	CString cmd = _T("cmd /c ") + CURRENT_PATH + _T("\\Resources\\ffmpeg\\ffmpeg.exe -codecs > ") + CURRENT_PATH + _T("\\Resources\\codecs.txt");
	TCHAR* cmdT = (TCHAR*)malloc(sizeof(TCHAR) * 256); // Convert CString to TCHAR.
	if (NULL == cmdT) {
		perror("can not malloc");
		OutputDebugString(_T("TCHAR syntax (cmdT) malloc failed.\n"));
	}
	else {
