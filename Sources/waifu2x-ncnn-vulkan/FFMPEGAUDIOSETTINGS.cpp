...
BOOL FFMPEGAUDIOSETTINGS::OnInitDialog()
{
	...
	if (preset == 1) {
		...
		FINALPARAM_A = L"ffmpeg" + HIDEBANNER_A + L" -i $InFile" + MAP_CHAPTERS_A + MAP_METADATA_A + AUDIOONLY_A + DISABLESTREAM_A + DISABLEDATA_A + AUDIOSTREAM_A + OVERWRITE_A + L" $OutFile";
		prm->SetWindowText(FINALPARAM_A);
	}
	else if (preset == 0) {
		CComboBox* combo1 = (CComboBox*)GetDlgItem(IDC_COMBO_PRESET_A);
		...
		FINALPARAM_A = L"ffmpeg -hide_banner -i $InFile -map_chapters -1 -map_metadata -1 -vn -sn -dn -c:a pcm_s24le -y $OutFile";
	}
	else {
		m_PRESET_A.SetCurSel(0);
		CComboBox* combo1 = (CComboBox*)GetDlgItem(IDC_COMBO_PRESET_A);
...
void FFMPEGAUDIOSETTINGS::OnCbnSelchangeComboPreset()
{
	...
	if (index1 == 0) {
		...
		prm->EnableWindow(TRUE);
		combo2->EnableWindow(TRUE);
		FINALPARAM_A = L"ffmpeg -i $InFile $OutFile";
		prm->SetWindowText(FINALPARAM_A);
	}
	else {
		owB->SetCheck(BST_UNCHECKED);
...
void FFMPEGAUDIOSETTINGS::OnBnClickedOk()
{
	...
	if (((CEdit*)GetDlgItem(IDC_EDIT_COMMAND_PARAM_A))->GetWindowTextLength() != 0) {
		//replaceOtherStr(xv_PARAM_A, L"$OutFile", OUTDIR_A + OUTNAME_A + L".wav");
